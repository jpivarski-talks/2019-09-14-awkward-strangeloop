...
% \item The ones with $m \sim m_{K_s} = 0.5\mbox{ GeV}/c^2$ are good candidates.
% \end{enumerate}
% \end{frame}
...
\item Generating candidates an \mintinline{sql}{INNER JOIN} ($n$ items from the same collection) or a \mintinline{sql}{CROSS JOIN} (items from different collections) \mintinline{sql}{ON t1.eventId == t2.eventId}.
